...
const inputClass = computed<string>(() => {
  ...
  if (invalidMessage.value) {
    return "bg-white !border-red-v2-500";
  }

  return "bg-white hover:border-purple-v2-200 focus-within:!border-purple-v2-500";
});
  ...
  <div class="flex flex-col gap-2">
    <div class="flex items-center justify-between">
      <label
          v-if="label"
          :for="id"
          class="flex items-center gap-0.5 font-rethink-sans font-medium text-xs text-neutral-700 !leading-4 tracking-[0.12px]">
        {{ label }}

        <span
            v-if="required"
            class="text-purple-v2-500">
          *
        </span>
      </label>
      ...
      <div
          ...
          class="font-inter font-medium text-neutral-400 text-[10px] !leading-4">
    ...
    <div
        ...
        class="flex items-center gap-2 h-12 w-full rounded-xl p-[15px] border border-solid border-neutral-200"
        :class="[inputClass]"
        @click="onFocus">
      <slot
          v-if="$slots.icon"
      ...
      <input
          ...
          :class="[invalidMessage ? 'text-red-v2-500' : input.length ? 'text-neutral-700' : 'text-neutral-400 focus:text-neutral-700']"
          @input="onInput" />
    ...
    <span
        v-if="invalidMessage"
        class="text-[10px] font-inter font-normal text-red-v2-500 !leading-3">
      {{ invalidMessage }}
    </span>
  </div>
</template>
